...
function MeScroll(a, b) {
	this.scrollDom = document.getElementById(a);
	this.options = b || {};
...
MeScroll.prototype.extendDownScroll = function(b) {
	var a = !!navigator.userAgent.match(/\(i[^;]+;( U;)? CPU.+Mac OS X/);
	MeScroll.extend(b, {
		use: true,
		auto: true,
...
MeScroll.prototype.extendUpScroll = function(a) {
	var b = typeof window.orientation == "undefined";
	MeScroll.extend(a, {
		use: true,
		auto: false,
		isLock: false,
...
MeScroll.prototype.initDownScroll = function() {
	...
	c.scrollDom.insertBefore(c.downwarp, c.scrollDom.firstChild);
	c.scrollDom.addEventListener("touchstart", b);
	c.scrollDom.addEventListener("mousedown", b);

	function b(f) {
		if(c.isScrollTo) {
			f.preventDefault()
		}
		c.startTop = c.scrollDom.scrollTop;
		if(c.optDown.mustToTop) {
			c.startY = f.touches ? f.touches[0].pageY : f.clientY
		}
	}
	c.scrollDom.addEventListener("touchmove", a);
	c.scrollDom.addEventListener("mousemove", a);

	function a(j) {
		if(c.startTop != null && c.scrollDom.scrollTop <= 0 && !c.isDownScrolling && (!c.isUpScrolling || (c.isUpScrolling && c.optUp.isBoth)) && !c.optDown.isLock) {
			if(c.optDown.mustToTop && c.startTop > 0) {
				return
			}
			var f = j.touches ? j.touches[0].pageY : j.clientY;
			if(!c.preY) {
				c.preY = f
			}
			var i = f - c.preY;
			c.preY = f;
			if(!c.startY && !c.optDown.mustToTop) {
				c.startY = f
			}
			var h = f - c.startY;
			if(h > 0) {
				...
				if(c.downHight < c.optDown.offset) {
					...
					if(i > 0) {
						c.downHight += i * c.optDown.outOffsetRate
					} else {
						c.downHight += i
					}
				}
				c.downwarp.style.height = c.downHight + "px";
				var g = c.downHight / c.optDown.offset;
				c.optDown.onMoving(c, g, c.downHight)
			}
		}
	}
	c.scrollDom.addEventListener("touchend", d);
	c.scrollDom.addEventListener("mouseup", d);
	c.scrollDom.addEventListener("mouseleave", d);
...
MeScroll.prototype.initUpScroll = function() {
	...
	a.scrollDom.appendChild(a.upwarp);
	a.scrollDom.addEventListener("scroll", function() {
		var c = a.scrollDom.scrollTop;
		if(!a.isUpScrolling && (!a.isDownScrolling || (a.isDownScrolling && a.optDown.isBoth))) {
			if(!a.optUp.isLock) {
				var b = a.scrollDom.scrollHeight - a.scrollDom.clientHeight - c;
				if(b <= a.optUp.offset) {
					a.triggerUpScroll()
				}
			}
			if(a.optUp.toTop.src) {
				if(c >= a.optUp.toTop.offset) {
					a.showTopBtn()
				} else {
					a.hideTopBtn()
				}
		...
		a.optUp.onScroll && a.optUp.onScroll(a, c)
	});
	setTimeout(function() {
...
MeScroll.prototype.endSuccess = function(b, d) {
	if(this.isDownScrolling) {
		this.endDownScroll()
	}
	if(this.optUp.use) {
		var e = this.optUp.page.num;
		var a = this.optUp.page.size;
		if(e == 1) {
			this.clearDataList()
		}
		var c;
...
MeScroll.prototype.loadFull = function() {
	var a = this;
	if(a.optUp.loadFull.use && !a.optUp.isLock && a.scrollDom.scrollHeight <= a.scrollDom.clientHeight) {
		setTimeout(function() {
			if(a.scrollDom.scrollHeight <= a.scrollDom.clientHeight) {
				a.triggerUpScroll()
			}
...
MeScroll.prototype.removeEmpty = function() {
	if(this.emptyDom) {
		var a = this.emptyDom.parentNode;
		if(a) {
...
//			document.body.appendChild(a.toTopBtn);
			document.getElementById("device_content").append(a.toTopBtn);
		}
...
MeScroll.prototype.scrollTo = function(h, k) {
	...
	var e = k / f;
	var g = this;
	var a = g.scrollDom.scrollHeight - g.scrollDom.clientHeight;
	if(h < 0) {
		h = 0
	}
	if(h > a) {
	...
	var d = window.setInterval(function() {
		if(c < e) {
			if(c == e - 1) {
				g.scrollDom.scrollTop = h
			} else {
				g.scrollDom.scrollTop -= b
			}
			c++
		} else {
			g.isScrollTo = false;
